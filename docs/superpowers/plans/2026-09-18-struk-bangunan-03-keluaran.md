...
- **Alur izin Bluetooth belum pernah dijalankan.** Entri manifest ada, tetapi bukan hanya UI-nya yang belum ada — **permukaan API untuk meminta izin pun belum ada**. `PrinterBluetooth` hanya menyediakan kueri boolean `izinDiberikan()` dan `bluetoothMenyala()`, sementara spec bagian 7 menjanjikan tombol "Izinkan", "Buka Pengaturan HP", dan "Nyalakan" yang semuanya menuntut aksi.
- **`PrinterBluetoothAsli` tidak punya test dan tidak punya pemanggil,** sama persis kategorinya dengan `_lewatShareSheet`. Satu titik di dalamnya bisa salah diam-diam: pemetaan `p.name` → `nama` dan `p.macAdress` → `mac`. Bila keduanya tertukar, tidak ada test yang gagal dan gejalanya baru muncul di tangan pengguna.
...
- **Menambah aksi izin Bluetooth wajib dilakukan dengan menambah metode ke antarmuka `PrinterBluetooth`,** bukan dengan mengimpor `print_bluetooth_thermal` dari `state/` atau `ui/`. Bila itu terjadi, aturan "hanya satu berkas menyentuh plugin" runtuh tanpa ada yang menyadari.
- **`ShareService.bagikan` membuang `ShareResult`,** sehingga Rencana 4 tidak bisa membedakan "pengguna benar-benar mengirim" dari "pengguna membatalkan share sheet". Spec bagian 7 menjanjikan tombol Kirim WA menjelaskan keadaan bila WhatsApp tidak terpasang; informasi untuk itu dibuang di sana.
- **`susunPdf` menerima `lebarKolom` terlepas dari dokumen yang menghasilkan PNG-nya.** Ketidakcocokan menghasilkan halaman berukuran salah secara diam-diam (isinya tetap benar karena `BoxFit.fitWidth`). Ambil keduanya dari `profil.lebarKolom` yang sama.
- **`PrinterBluetooth.putus()` belum punya satu pun pemanggil** — setiap perangkat palsu wajib mengimplementasikannya tanpa alasan. Pakai atau buang saat Rencana 4 menentukan daur hidup sambungan.
- **Pemetaan "lebar kolom → kertas fisik" hidup di tiga tempat:** `ProfilToko.lebarKolom`, `susunEscPos`, dan `lebarHalamanPdf`. Ambangnya konsisten, jadi ini bukan bug — tetapi spec bagian 3 berkata penyaji tidak boleh tahu lebar kertas, dan di sinilah pengetahuan itu bocor dua kali.
